...
const cookie_parser_1 = __importDefault(require("cookie-parser")); // ✅ import correto
...
app.use((0, cookie_parser_1.default)()); // ✅ sem require()
// Rotas para teste
...
describe('Auth Controller', () => {
    ...
    describe('POST /register', () => {
        ...
        it('deve retornar erro quando campos obrigatórios estão ausentes', async () => {
            const response = await (0, supertest_1.default)(app).post('/register').send({
                email: 'joao@teste.com',
                password: 'MinhaSenh@123',
                // fullName ausente
            });
            expect(response.status).toBe(400);
            expect(response.body.error).toBe('fullName, email e password são obrigatórios');
        });
        ...
        it('deve retornar erro 500 quando ocorre erro no servidor', async () => {
            mockUserModel.findOne.mockResolvedValue(null);
            mockUserModel.create.mockRejectedValue(new Error('Erro no banco de dados'));
            const response = await (0, supertest_1.default)(app)
                .post('/register')
                .send(validRegistrationData);
    ...
    describe('POST /login', () => {
        ...
        it('deve fazer login com sucesso', async () => {
            ...
            const response = await (0, supertest_1.default)(app).post('/login').send(validLoginData);
    ...
    describe('POST /refresh', () => {
        it('deve renovar o token com sucesso', async () => {
            const validToken = jsonwebtoken_1.default.sign({ sub: mockUser.id }, process.env.JWT_REFRESH_SECRET);
        ...
        it('deve retornar erro quando usuário não é encontrado', async () => {
            const validToken = jsonwebtoken_1.default.sign({ sub: 'invalidUserId' }, process.env.JWT_REFRESH_SECRET);
    ...
    describe('POST /logout', () => {
        it('deve fazer logout com sucesso', async () => {
            const response = await (0, supertest_1.default)(app).post('/logout');
            expect(response.status).toBe(200);
